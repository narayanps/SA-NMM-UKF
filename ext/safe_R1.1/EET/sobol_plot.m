function sobol_plot(mi,sigma,varargin)
%
...
%fn = 'Courier' ;
fs = 28 ; % font size of axes, labels, etc.
ms = 22 ; % marker size
...
nb_legend = 10  ; % number of input names that will be displayed in the legend
...
L=ceil(M/A);
clrs=repmat(col,L,1);
clrs(1,:)=hex2rgb('#DA8A8F');
clrs(2,:)=hex2rgb('#FFFF00');
clrs(3,:)=hex2rgb('#8BABF1');
clrs(4,:)=hex2rgb('#F57600');
clrs(5,:)=hex2rgb('#C44601');
clrs(6,:)=hex2rgb('#DC17FF');
clrs(7,:)=hex2rgb('#5BA300');
clrs(8,:)=hex2rgb('#A8AA00');
clrs(9,:)=hex2rgb('#D1AB00');
clrs(10,:)=hex2rgb('#FA1637');
...
% Create legend:
my_lgd=legend(labelinput_new, 'Location', 'southwestoutside', 'Interpreter', 'latex');
pos = get(my_lgd,'position');
%set(my_lgd, 'Box', 'off')
set(my_lgd, 'position',[0.925 0.4 pos(3:4)]);
%my_lgd.NumColumns=5;
xlabel('First-order index','FontSize',fs,'FontName',fn)
ylabel('Total-order index','FontSize',fs,'FontName',fn)
grid on
